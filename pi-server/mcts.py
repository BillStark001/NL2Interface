...
class SingleStepMCTS:
    def __init__(self, state, task, root, proc, n_iters, start_time, max_iters):
        ...
        self.iter_per_merge = Exp.iter_per_merge
        self.uct_c = Exp.uct_c
        self.rand_map = Exp.rand_map
        self.root = root
        self.proc = proc
...
class MCTS:
    def __init__(self, state, task):
        ...
        self.restore = task.restore
        self.eval = task.evaluate
        self.n_procs = Exp.proc_number
        self.start_time = time.time()

        self.root = MCTSNode(None, -1, Exp.uct_c)
    ...
    def can_stop(self, heartbeats):
      ...
      for pairs in heartbeats:
        if len(pairs) < Exp.early_stop: return False
        for rec in pairs[-Exp.early_stop:]: costs.add(rec[1][1])
    ...
    def play(self):
      ...
      tasks = [
          SingleStepMCTS(
              ...
              self.start_time, Exp.iter_number
          )
      ...
      if self.n_procs == 1:
        ...
        random.seed(seeds[0])
        for i in range(Exp.iter_number):
    ...
    def merge_sub_results(self, roots):
        def merge(nodes):
            ...
            merged = MCTSNode(None, -1, Exp.uct_c)
            for n in nodes:
