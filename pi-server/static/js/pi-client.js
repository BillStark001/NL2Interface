...
(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.pi = global.pi || {}));
...
function BackendLibrary(socket) {
  function backendLib() {}
...
function project(data, mappings) {
 ...
 * @param {*} _vegaEmbed
 */
function initializeVega(_vegaLite, _vega, _vl, _vegaEmbed) {
  vl$1 = _vl;
  vegaEmbed$1 = _vegaEmbed;
}

// Adds flexbox properties to a div component. 
function makeFlex(flexBox, orientation){
  flexBox = jquery(flexBox);
...
function showInteractionHint(interaction_msg) {
  let int_div = jquery("<div>");
  int_div.append("<h4>Interactions:</h4><p>" + interaction_msg + "</p>");
  jquery("#interaction-hint").append(int_div);
}

function emptyInteractionHint() {
  jquery("#interaction-hint").empty();
}

function brushEventListener(vegaview, event, iact) {
...
function selectEventListener(vegaview, event) {
...
function panZoomEventListener(vegaview, event) {
  let canvas = event.srcElement;
  let mouseX = (event.clientX - canvas.getBoundingClientRect().left) + vegaview.scenegraph().root.bounds.x1;
  console.log(mouseX);
  let mouseY = event.clientY - canvas.getBoundingClientRect().top + vegaview.scenegraph().root.bounds.y1;
...
let makeVLRenderer = (view) => {
  let render = ( (table, onViewLoad) => {
    ...
    if (!view.vlview) {
      ...
      vegaEmbed$1(view.domEl, vlspec, {renderer: "canvas"})
        .then((vlview) => {
          view.vlview = vlview;
    ...
    */
    button.create = ((spec, domEl, wf) => {
      ...
      view.render = ((table) => {
        let data = project(table, view.mapping);
        data.forEach((row) => {
          let button = jquery(`<button class="btn btn-outline-primary"
            value='${row.value}'>${row.label}</button>`);
          jquery(view.domEl).append(button);
          button.on("click", () => {
            view.emit("interact", view);
            view.source_iact.forEach((iact) => {
              iact.trigger([{
    ...
    */
    radio.create = ((spec, domEl, wf) => {
      ...
      view.render = ((table) => {
        ...
        data.forEach((row,i) => {
          let radio = jquery(`<input class='form-check-input' 
          ...
          let div = jquery("<div class='form-check'/>");
          if (orientation == "H")
            div.addClass("form-check-inline");
          div.append(radio).append(label);
          jquery(view.domEl).append(div);
...
function Label() {
  ...
  label.create = ((spec, domEl, wf) => {
    ...
    view.spec = spec;
    view.domEl = domEl["parent"];
    view.dom = jquery(view.domEl);
    view.id = spec["id"];

    // render
    let label = jquery(`<span>${spec.data}</span>`);
    ...
    */
    checkbox.create = ( (spec, domEl, wf) => {
      ...
      view.render = ((table) => {
        ...
        data.forEach( (row, i) => {
          let checkbox = jquery(`<input class="form-check-input" 
            type="checkbox" id="${view.id}-${i}" name="${view.id}"
            value="${row.value}"/>`);
          let label = jquery(`<label class="form-check-label" 
            for="${view.id}-${i}">${row.label}</label>`);
          let div = jquery("<div class='form-check form-check-inline'/>");
...
function Bar() {
  ...
  */
  bar.create = ( (spec, domEl, wf) => {
    ...
    view.source_iact = [];
    view.mark = vl$1.markBar({tooltip: { content: "data" }}).data([]);
    ...
    view.addSourceIact = ((iact) => {
      ...
      let sel;
      if (iact.type === "SINGLE") 
        sel = vl$1.selectSingle();
      else if (iact.type === "MULTI")  {
        sel = vl$1.selectPoint();
        sel.empty(false);
        view.mark = view.mark.encode(vl$1.color().value('lightgray').if(sel, vl$1.color().value('#4682b4')));
        iact.selected = [];
      }
      else
        sel = vl$1.selectInterval().encodings(['x']);
...
function Line() {
  ...
  */
  line.create = ( (spec, domEl, wf) => {
    ...
    view.source_iact = [];
    view.manipulationSchema = (m) => {};
    view.mark = vl$1.markLine({'point': true, tooltip: { content: "data" }}).data([]);
    ...
    view.addSourceIact = ((iact) => {
      ...
      var sel;
      if (iact.type === "SINGLE") 
        sel = vl$1.selectSingle().nearest(true);
      else if (iact.type === "MULTI") 
        sel = vl$1.selectMulti().toggle(false).nearest(true);
      else if (iact.type === "BRUSHX") 
        sel = vl$1.selectInterval().encodings(['x']);
      else if (iact.type === "BRUSHY") 
        sel = vl$1.selectInterval().encodings(['y']);
      else if (iact.type === "BRUSHXY") 
        sel = vl$1.selectInterval().encodings(['x', 'y']);
      else if (iact.type === "PANX") 
        sel = vl$1.selectInterval().bind("scales").encodings(['x']).zoom(false);
      else if (iact.type === "PANY") 
        sel = vl$1.selectInterval().bind("scales").encodings(['y']).zoom(false);
      else if (iact.type === "PANXY") 
        sel = vl$1.selectInterval().bind("scales").encodings(['x', 'y']).zoom(false);
      else if (iact.type === "ZOOMX") 
        sel = vl$1.selectInterval().bind("scales").encodings(['x']).zoom(true);
      else if (iact.type === "ZOOMY") 
        sel = vl$1.selectInterval().bind("scales").encodings(['y']).zoom(true);
      else if (iact.type === "ZOOMXY") 
        sel = vl$1.selectInterval().bind("scales").encodings(['x', 'y']).zoom(true);
      else 
...
function Point() {
  ...
  */
  point.create = ( (spec, domEl, wf) => {
    ...
    view.backend = wf.backends[spec["data"]["backend"]];
    view.source_iact = [];
    view.mark = vl$1.markPoint({filled: true, tooltip: { content: "data" }}).data([]);
    ...
    view.mark = view.mark
      ...
      .height(spec.height || view.domEl.height)
      .autosize({'type': 'fit', 'resize': true, 'contains': 'padding'});
    view.mark = view.mark.encode(vl$1.size().value(80));
    ...
    view.addSourceIact = ((iact) => {
      ...
      view.mark = view.mark.encode(vl$1.size().value(100));
...
function Circle() {
  ...
  */
  circle.create = ( (spec, domEl, wf) => {
    ...
    view.backend = wf.backends[spec["data"]["backend"]];
    view.source_iact = [];
    view.mark = vl$1.markCircle({filled: false, tooltip: { content: "data" }}).data([]);
...
function Square() {
  ...
  */
  square.create = ( (spec, domEl, wf) => {
    ...
    view.backend = wf.backends[spec["data"]["backend"]];
    view.source_iact = [];
    view.mark = vl$1.markSquare({filled: false, tooltip: { content: "data" }}).data([]);
...
function Area() {
  ...
  */
  area.create = ( (spec, domEl, wf) => {
    ...
    view.source_iact = [];
    view.manipulationSchema = (m) => {};
    view.mark = vl$1.markArea({'point': true, tooltip: { content: "data" }}).data([]);
...
function Geoshape() {
  ...
  geoshape.create = ( (spec, domEl, wf) => {
    ...
    view.backend = wf.backends[spec["data"]["backend"]];
    view.source_iact = [];
    view.mark = vl$1.markGeoshape({filled: true, tooltip: { content: "data" }}).data([]);
    ...
    for (let key in view.mapping) {
      ...
      if (typ === "ordinal") {
        ...
        view.mark = view.mark.project(vl$1.projection('albersUsa'));
      }
      else {
        if (label === "date")
          view.mark = view.mark.encode(vl_fields[key]().fieldT(key).title(label));
    ...
    view.addSourceIact = ((iact) => {
      ...
      if (iact.type === "SINGLE") {
        sel = vl$1.selectSingle();
        sel.empty(false);
        view.mark = view.mark.encode(vl$1.size().value(80), vl$1.color().value('lightgray').if(vl$1.not(sel), vl$1.color().fieldQ("color").scale({"zero": false, "scheme": "reds"
              }))).params(sel);
        iact.selected = [];
      }
      else if (iact.type === "MULTI") {
        sel = vl$1.selectPoint();
        sel.empty(false);
        view.mark = view.mark.encode(vl$1.size().value(80), vl$1.color().value('lightgray').if(vl$1.not(sel), vl$1.color().fieldN("color"))).params(sel);
        iact.selected = [];
      }
      else 
        alert("interaction not supported " + iact.type);
...
function Toggle() {
    ...
    */
    toggle.create = ((spec, domEl, wf) => {
      ...
      view.render = ((table) => {
        let el = jquery(`<label class="switch">
          <input type="checkbox" value='0'/>
          <span class="slider"></span>
        </label>`);
        view.toggle = el.find("input");
        jquery(view.domEl).append(el);
        if ("children" in domEl)
            jquery(view.domEl).append(domEl["children"][0]);
...
function build_layout(spec, wf) {
  let div = jquery(`<div id="${spec.id}" ></div>`)
    .css({ 
      width: `${spec.width}px`,
...
function ViewLibrary() {
  ...
  viewLib.buildLayout = ((specRoot, wf) => {
    jquery(wf.div).append(build_layout(specRoot, wf));
  ...
  viewLib.createView = ( (spec, wf) => {
    ...
    if (typ == "Button") {
      let parent = jquery(domEl.parentNode);
...
function PIWorkflow$1(socket, session, config = defaultConfig) {
  function workflow() {}
  ...
  workflow.init = (spec, div) => {
    ...
    if (config.enableHelp) {
      ...
      jquery("#draggable-help").dialog({
        autoOpen: false,
      });
      jquery("#widget-help-btn").click(function () {
        if (!jquery("#draggable-help").dialog("isOpen"))
          jquery("#draggable-help").dialog("open");
      });
    }
...
function PIWorkflow(socket, session, config) {
  // Initialize vega from included <script> globals
  initializeVega(null, vega, vl, vegaEmbed);
  return PIWorkflow$1(socket, session, config);
}

exports.PIWorkflow = PIWorkflow;
