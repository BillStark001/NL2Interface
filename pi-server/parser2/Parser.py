from antlr4 import *
from parser2.DiffSqlLexer import DiffSqlLexer
from parser2.DiffSqlParser import DiffSqlParser
import json
import re
import copy
...
if __name__ == "__main__":
    ...
    #root = parse("SELECT Multi{Any{a,b},,}, c, Subset{,, d, e} Opt{, Any{f,g}} ", compact=True)
    # root = parse("select distinct a from T", compact=False)
    root = parse_difftree("SELECT Any{a, b}, Any{c, d; default = c1}, Multi{,,x; default = 7} from T where a = Any{1,2; default = 11} order by Any{e, f; default = e1} Opt{limit 4; default = 'limit 4'}", compact=False)
    # root = parse("SELECT Any{a, b; default = a1}, Any{c, d; default = c1} from T where a = Any{1,2; default = 11} Opt{order by a; default = f}", compact=False)
