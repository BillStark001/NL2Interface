...
export function Geoshape() {
  ...
  geoshape.create = ( (spec, domEl, wf) => {
    ...
    view.source_iact = [];
    view.mark = vl.markGeoshape({filled: true, tooltip: { content: "data" }}).data([]);
    ...
    view.addSourceIact = ((iact) => {
      ...
      if (iact.type === "SINGLE") {
        sel = vl.selectSingle();
        sel.empty(false);
        view.mark = view.mark.encode(vl.size().value(80), vl.color().value('lightgray').if(vl.not(sel), vl.color().fieldQ("color").scale({"zero": false, "scheme": "reds"
              }))).params(sel);
        iact.selected = [];
      }
